...
Pour inspecter il existe une premier solution. En déclaration la liaison entre un contrat et son implémentation dans la configuration, une méthode Setter permet de spécifier pour quelle type de dépendance sous jacente l'injection peut être faite : 
...
Sans vous en rendre compte vous venez ici de déloquer une possibilité du Container. En effet 
