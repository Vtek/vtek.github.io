...
Pour inspecter il existe une premier solution. En déclaration la liaison entre un contrat et son implémentation dans la configuration, une méthode Setter permet de spécifier une propriété à injecter : 
...
Setter<T> dispose d'une surcharge qui permet d'injecter à la condition que le nom de la propriété match avec celui passé en argument :
