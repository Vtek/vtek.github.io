...
NOTE: Les fonctions fléchées sont normalisées dans *ES*. Elles sont utilisées depuis longtemps en programmation, et on les retrouvent souvent sous le nom d'*Expression Lambda*. Cette dénomination vient du travail d'_Alonzo Church_ qui est l'inventeur de ce concept (https://fr.wikipedia.org/wiki/Lambda-calcul[λ-calcul]). Il est possible de les utiliser dans beaucoup de langage, notamment *C#*, *Lisp*, *Python* et même *C++* depuis la version 11 !